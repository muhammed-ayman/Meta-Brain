...
out = cv.VideoWriter('outpy2.avi',cv.VideoWriter_fourcc('M','J','P','G'), 10, (640,480))
...
while True:
    ...
    #image, faces = detector.findFaceMesh(frame, draw=True)
    #out.write(image)
    if ret == False:
        break
    ...
    image, face = m.faceDetector(frame, grayFrame, Draw=True)
    if face is not None:
